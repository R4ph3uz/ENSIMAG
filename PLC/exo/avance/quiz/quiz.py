...
class Quiz:
    def __init__(
        ...
        with open(yaml_filename, "r") as data_file:
            self.raw_data = yaml.safe_load(data_file)

        with open(progress_filename, "r") as progress_file:
            self.progress = yaml.safe_load(progress_file)
        ...
        for q in self.raw_data["questions"]:
            question = Question(
                QTYPES.MULTIPLE, q["title"], q["description"], q["answers"]
            )
        ...
        self.tot_score = (
            tot_score
            if tot_score > 0
            else sum([len(q["answers"]) for q in self.raw_data["questions"]])
        )
    ...
    def save_progress(self):
        ...
        for q in self.questions:
            ...
            for id, _ in enumerate(q.answers):
                question_progress["answers"][id] = (
                    "correct" if q.is_correct(id) else "wrong"
                )
        ...
        if self.use_git:
            push_files(
                [self.progress_filename], "[progress] Quiz {}".format(self.title)
            )
